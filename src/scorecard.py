...
from .config import BASE_GOOD_BAD_ODDS, BASE_SCORE, CANDIDATE_FEATURES, PDO, TEST_SIZE
...
def fit_pd_scorecard(dataset: pd.DataFrame, seed: int) -> dict:
    ...
    if len(selected_features) < 8:
        selected_features = iv_summary.head(min(10, len(iv_summary)))["feature"].tolist()
